...
class EIClusteredNetwork:
    ...
    def setup_network(self, cluster_strength):
        ...
        self.connect_network(cluster_strength)
    ...
    def get_simulation(self, cluster_strength, PathSpikes=None):
        self.setup_network(cluster_strength)
        self.simulate()
...
cluster_strengths = range(1, 21)  # from 1 to 20
...
# Iterate over each cluster strength and run simulation
for strength in cluster_strengths:
    simulation_results = ei_clustered_network.get_simulation(strength, PathSpikes=f"spike_times_cluster_strength_{strength}.pkl")
    spike_times = simulation_results['spiketimes']
    plot_spike_raster(exc_weights, inh_weights, spike_times, network_params["simtime"], strength)
